...
if __name__ == '__main__':
    ...
    DATASET = os.path.join(HOME, 'data/datasets/CIFAR100')
    ...
    images = val_raw[b'data'].reshape(n_train, channel, height, width)
    ...
    images = val_raw[b'data'].reshape(n_test, channel, height, width)
    images = np.swapaxes(images, 1, 2)
    images = np.swapaxes(images, 2, 3)
    labels = np.array(val_raw[b'fine_labels'], dtype=np.uint8)
