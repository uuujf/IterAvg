...
def PGDPath(model, lr, l2regu, num_iters):
    opt_path = np.zeros((num_iters, 2))
    model.initialization()
    for i in range(num_iters):
        opt_path[i] = model.w.detach().numpy().copy()

        loss = model.loss() + model.generalL2(l2regu)
        dw = -lr * grad(loss, model.w)[0]
        dw = torch.matmul(dw, model.Q_inv)
        model.w.data += dw
...
if __name__ == "__main__":
    ...
    # paths
    w = PGDPath(model, eta, 0, num_iters)
    np.save(w_name, w)

    what = PGDPath(model, gamma, l2regu, num_iters)
    ...
    # plot paths
    plt.scatter(w[:,0], w[:,1], c='g', s=1.5, label='PGD Path $w_k$')
    plt.scatter(what[:,0], what[:,1], c='b', s=1.5, label='Regularized PGD Path $\hat{w}_k$')
    plt.scatter(wtilde[:,0], wtilde[:,1], c='r', s=1.5, label='Averaged PGD Path $\\tilde{w}_k$')

    plt.legend(loc='lower right', markerscale=8, fontsize=16)
